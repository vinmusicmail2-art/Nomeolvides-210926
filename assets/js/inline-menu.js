(() => {
  ...
  const categories = [
    { id: 'ensaladas', label: 'Ensaladas', note: 'Frescas y para compartir.', icon: '❦' },
    { id: 'tapas', label: 'Tapas y entrantes', note: 'Pequeños bocados, grandes encuentros.', icon: '☙' },
    { id: 'carnes', label: 'Carnes', note: 'Sabores intensos en cada plato.', icon: '♉' },
    { id: 'pescados', label: 'Pescados y mariscos', note: 'Del mar a la mesa.', icon: '♓' },
    { id: 'pasta', label: 'Pasta', note: 'Recetas para compartir.', icon: '◎' },
    { id: 'infantil', label: 'Menú infantil', note: 'Opciones pensadas para los pequeños.', icon: '✦' }
  ];
  ...
  function categoryTabs() {
    return categories.map((category) => `<button class="inline-menu__tab${category.id === selectedCategory ? ' is-active' : ''}" type="button" data-category="${category.id}" aria-pressed="${category.id === selectedCategory}"><span class="inline-menu__tab-icon" aria-hidden="true">${escapeHtml(category.icon)}</span><span>${escapeHtml(category.label)}</span></button>`).join('');
  }

  function dishCard(dish, mode = 'full') {
    const prompt = dish.amount === null ? 'Consultar para delivery' : 'Añadir a delivery';
    return `<article class="dish-card dish-card--${mode}" data-open-dish="${dish.id}">
  ...
  function render() {
    ...
    if (menuExpanded) {
      ...
      const reveal = root.querySelector('.inline-menu__reveal');
      renderFullMenu();
      full.hidden = false;
  ...
  root.addEventListener('click', (event) => {
    ...
    if (!target) {
      const card = event.target.closest('[data-open-dish]');
      if (card) openDish(card.dataset.openDish);
      return;
    }
